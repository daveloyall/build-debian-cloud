...
class Volume(object):
	...
	events_none = [{'name': 'create', 'src': 'nonexistent', 'dst': 'detached'},
	               {'name': 'attach', 'src': 'detached', 'dst': 'attached'},
	               {'name': 'format', 'src': 'attached', 'dst': 'formatted'},
	               {'name': 'mount', 'src': ['formatted', 'mounted'], 'dst': 'mounted'},
	               {'name': 'unmount', 'src': 'mounted', 'dst': 'formatted'},
	               {'name': 'detach', 'src': 'formatted', 'dst': 'detached_fmt'},
	               {'name': 'delete', 'src': ['detached', 'detached_fmt'], 'dst': 'deleted'},

	               {'name': 'attach', 'src': 'detached_fmt', 'dst': 'formatted'},
	               {'name': 'detach', 'src': 'attached', 'dst': 'detached'},
	               ]

	events_prt = [{'name': 'create', 'src': 'nonexistent', 'dst': 'detached'},
	              {'name': 'attach', 'src': 'detached', 'dst': 'attached'},
	...
	mount_events = [{'name': 'mount_root', 'src': 'unmounted', 'dst': 'root_mounted'},
	                {'name': 'mount_specials', 'src': 'root_mounted', 'dst': 'specials_mounted'},
	                {'name': 'unmount_specials', 'src': 'specials_mounted', 'dst': 'root_mounted'},
	                {'name': 'unmount_root', 'src': 'root_mounted', 'dst': 'unmounted'}
	                ]

	mount_events_boot = [{'name': 'mount_boot', 'src': 'specials_mounted', 'dst': 'boot_mounted'},
	                     {'name': 'unmount_boot', 'src': 'boot_mounted', 'dst': 'specials_mounted'},
	                     {'name': 'unmount_specials', 'src': 'boot_mounted', 'dst': 'boot_mounted_no_specials'},

	                     {'name': 'mount_boot', 'src': 'root_mounted', 'dst': 'boot_mounted_no_specials'},
	                     {'name': 'unmount_boot', 'src': 'boot_mounted_no_specials', 'dst': 'root_mounted'},
	                     ]
	...
	def __init__(self, partition_map, events=[], callbacks={}):
		...
		if isinstance(self.partition_map, NoPartitions):
			events += self.events_none
			callbacks['onafterattach'] = lambda e: self.partition_map.create(self)
		else:
			events += self.events_prt

		self.fsm = Fysom({'initial': 'nonexistent',
		                  'events': events,
		                  'callbacks': callbacks})

		if hasattr(self.partition_map, 'boot'):
			mount_events = self.mount_events + self.mount_events_boot
		else:
		...
		attach_proxy_methods(self, events, self.fsm)
		attach_proxy_methods(self, mount_events, self.mount_fsm)
	...
	def _unmount(self, e):
		if self.mount_fsm.current is 'boot_mounted':
			self.unmount_boot()
		if self.mount_fsm.current is 'boot_mounted_no_specials':
			self.unmount_boot()
